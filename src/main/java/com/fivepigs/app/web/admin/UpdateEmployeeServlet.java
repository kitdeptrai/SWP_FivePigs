...
@WebServlet("/admin/employees/update")
public class UpdateEmployeeServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (userIdStr == null || fullName == null || status == null || roleName == null ||
            ...
            resp.sendRedirect(req.getContextPath() + "/admin/employees?error=missing_fields");
            return;
        }
        ...
        try {
            ...
            resp.sendRedirect(req.getContextPath() + "/admin/employees?error=invalid_id");
            return;
        }

        // allow only reviewer / Approval
        String roleLower = roleName.trim().toLowerCase();
        if (!(roleLower.equals("reviewer") || roleLower.equals("approval") || roleLower.equals("aproval"))) {
            resp.sendRedirect(req.getContextPath() + "/admin/employees?error=invalid_role");
            return;
        }

        String st = status.trim().toUpperCase();
        if (!(st.equals("ACTIVE") || st.equals("INACTIVE"))) {
            resp.sendRedirect(req.getContextPath() + "/admin/employees?error=invalid_status");
            return;
        }
