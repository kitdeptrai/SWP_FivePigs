...
@WebServlet(name = "LicenseUnshareServlet", urlPatterns = {"/license/unshare"})
public class LicenseUnshareServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        Integer targetUserId = parseInt(request.getParameter("targetUserId"));
        if (softwareId == null || targetUserId == null) {
            response.sendRedirect(request.getContextPath() + "/library?shareMsg=invalid_user");
            return;
        }

        try {
            String result = licenseDao.removeSharedUser(ownerId, softwareId, targetUserId);
